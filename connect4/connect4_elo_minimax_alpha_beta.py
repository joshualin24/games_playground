...
# --- Configuration ---
GAMES_PER_MATCHUP = 10  # Games played per pair of agents
BOARD_ROWS = 6
BOARD_COLS = 7
EMPTY = 0
P1_PIECE = 1
...
# Alpha-Beta optimization: Search center columns first to maximize pruning
...
class Connect4Sim:
    ...
    def get_valid_locations(self, board):
        # Return valid locations sorted by center-proximity (Move Ordering)
        return [c for c in CENTER_ORDER if self.is_valid_location(board, c)]
    ...
    def evaluate_window(self, window, piece):
        ...
        if window.count(opp_piece) == 3 and window.count(EMPTY) == 1:
            score -= 80 # Heavy penalty for letting opponent get 3-in-a-row
    ...
    def run_minimax(self, player_idx):
        ...
        if depth == 0: return random.choice(valid_locations)

        col, score = self.minimax_recursive(self.board, depth, -math.inf, math.inf, True, piece)
        
        if col is None: col = random.choice(valid_locations)
        return col
...
def main():
    agents = [
        {'id': 1, 'name': "Depth 1 (Novice)", 'depth': 1, 'elo': 1200},
        {'id': 2, 'name': "Depth 2 (Amateur)", 'depth': 2, 'elo': 1200},
        {'id': 3, 'name': "Depth 3 (Pro)", 'depth': 3, 'elo': 1200},
        {'id': 4, 'name': "Depth 4 (Master)", 'depth': 4, 'elo': 1200},
        {'id': 5, 'name': "Depth 5 (Grandmaster)", 'depth': 5, 'elo': 1200}
    ]

    print(f"Starting TRUE Connect 4 Tournament ({GAMES_PER_MATCHUP} Games Per Matchup)")
    print("-" * 75)
    print(f"{'Matchup':<45} | {'Winner':<20} | {'Turns'}")
    print("-" * 75)
    ...
    for i in range(len(agents)):
        for j in range(i + 1, len(agents)):
            ...
            for game_num in range(GAMES_PER_MATCHUP):
                ...
                matchup_str = f"Game {game_num+1}: {current_p1['name']} vs {current_p2['name']}"
                print(f"{matchup_str:<45} | {winner_name:<20} | {turns}")
